...
format_date() {
    date_string="$1"
    formatted_date=$(date -d "$date_string" "+%d %B %Y à %Hh%M") || return 1
    echo "$formatted_date"
    return 0
}
############################################################################################################################
check-new-conf-file() {
    ...
    offline_epoch=$(date --date="$offline_timestamp" +%s 2>/dev/null) || offline_epoch=0
    ...
    grep -E "\.rpm(new|save|orig)" "$rpm_log" | while IFS= read -r line; do
        ...
        if [[ -z "$line_timestamp" || ! "$line_timestamp" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2} ]]; then
			{ echo ; echo "Avertissement : Pas de timestamp valide trouvé pour la ligne suivante :" ; grep -A5 "$line" "$rpm_log" ; } >> "$bigerror" || true
           	continue
        fi
        # Convertir le timestamp et gérer les erreurs
        line_epoch=$(date --date="$line_timestamp" +%s 2>/dev/null) || {
            echo "Erreur de conversion du timestamp pour la ligne : " >> "$bigerror" || true
            grep -A5 "$line" "$rpm_log" >> "$bigerror" || true
            continue
        }
        ...
        if [ "$time_diff" -le 60 ]; then
            ...
            if [ -f "$config_file" ]; then
            	echo -e "\n==> Nouveau fichier de configuration trouvé ($config_file) installé le $offline_timestamp." >> "$success" || true
            fi
        fi
    done
}
...
if [ -f "$indicator_file" ]; then
    ...
    while ! dbus-send --session --dest=org.freedesktop.Notifications --type=method_call \
        ...
        if [ $attempt -ge $max_attempts ]; then
            echo "Avertissement : D-Bus n'est toujours pas prêt après $attempt tentatives. Continuation du script quand même." > "$bigerror" || true
            break
        fi
        sleep 5 || true
    done
    sleep 5 || true
    last_result=$(tac "$log_file" | grep -m 1 -E 'Complete!|Terminé|err') || last_result="Résultat inconnu"
    update_time=$(echo "$last_result" | awk '{print $1}') || update_time=""
    readable_time=$(format_date "$update_time")
    last_result=${last_result,,}

    if [[ $last_result == *"complete"* || $last_result == *"terminé"* ]]; then
        grep "${update_time}" "$log_file" | awk 'NR==1 {print $1, $3, $4, $5, $6, $7, $8, $9} NR>1 {$1=$2=""; print $0}' > "$success" || true
        check-new-conf-file
        notify-send -i "$icon" --app-name "Mise à jour hors-ligne" "Effectuée correctement le $readable_time." "Fichier de log : $success" || true
    elif [[ $last_result == *"err"* ]]; then
        grep "${update_time}" "$log_file" > "$failure" || true
        notify-send -i "$icon" -u critical --app-name "Mise à jour hors-ligne" "En échec le $readable_time." "Fichier de log : $failure" || true
    else
        grep "${update_time}" "$log_file" > "$failure" || true
        notify-send -i "$icon" -u critical --app-name "Mise à jour hors-ligne" "Résultat inconnu." "Fichier de log : $failure" || true
    fi
    rm -f "$indicator_file" || true
fi
